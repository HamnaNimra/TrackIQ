...
def simulate_inference(batch_size: int, base_overhead: float = 0.01, time_per_image: float = 0.005) -> float:
    """
...
def analyze_tradeoff(total_images: int, batch_sizes: List[int]):
    ...
    print("Base overhead: 10ms (kernel launch)")
    print("Per-image processing: 5ms\n")
    ...
    print(
        f"• Lowest latency:     batch_size={min_latency['batch_size']} ({min_latency['latency']:.2f}ms per image)"
    )
    print(
        f"• Highest throughput: batch_size={max_throughput['batch_size']} "
...
def main():
    ...
    parser.add_argument("--images", type=int, default=100, help="Total images to process (default: 100)")
    parser.add_argument(
        "--batch-sizes",
        type=int,
        nargs="+",
