...
class PowerGauge:
    ...
    def __init__(
        ...
        live_device: Optional[DeviceProfile] = None,
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if (
            self.live_device is not None
            and self.live_device.device_type == DEVICE_TYPE_NVIDIA_GPU
        ):
            perf = get_performance_metrics()
            payload["live_power_watts"] = (
                float(perf.get("power")) if perf and perf.get("power") is not None else None
            )
        if (
