...
class PerformanceVisualizer:
    ...
    def plot_memory_transfer_timeline(
        ...
        if h2d_times:
            ax.bar(
                x[: len(h2d_times)] - width / 2,
                h2d_times,
                width,
                label="H2D (Host→Device)",
                color="#3498DB",
                alpha=0.8,
            )
        if d2h_times:
            ax.bar(
                x[: len(d2h_times)] + width / 2,
                d2h_times,
                width,
                label="D2H (Device→Host)",
                color="#E74C3C",
                alpha=0.8,
            )
    ...
    def plot_tegrastats_overview(
        ...
        ax3.set_title(
            f"Memory: {mem_used:.0f}MB / {mem_total:.0f}MB ({mem_pct:.1f}%)",
            fontsize=12,
            fontweight="bold",
        )
