...
class LLMMemoryCalculator:
    ...
    def calculate_kv_cache(self, batch_size, context_length):
        ...
        kv_size_per_layer = (
            2 * batch_size * self.num_heads * context_length * self.head_size * self.bytes_per_param  # K and V
        )
...
# Example usage
calculator = LLMMemoryCalculator(num_params_billions=70, num_layers=80, num_heads=64, head_size=128, precision="fp16")

print("=== 70B Model Memory Requirements ===\n")

# Check if it fits
mem = calculator.calculate_total(batch_size=1, context_length=128000)
print("Batch size: 1, Context length: 128k tokens")
